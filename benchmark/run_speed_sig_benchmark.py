...
def parse_speed_sig_output(text):
    ops = {}
    sizes = {}
    lines = text.splitlines()
    for line in lines:
        m = re.match(r"^(keypair|sign|verify)\s*\|(.*)$", line)
        if m:
            op = m.group(1)
        ...
        if "public key bytes:" in line:
            m2 = re.search(
                r"public key bytes:\s*(\d+),\s*secret key bytes:\s*(\d+),\s*signature bytes:\s*(\d+)",
                line,
            )
            if m2:
                sizes = {
                    "public_key_bytes": int(m2.group(1)),
                    "secret_key_bytes": int(m2.group(2)),
                    "signature_bytes": int(m2.group(3)),
                }
    return ops, sizes
    ...
    for alg in algorithms:
        ...
        for r in range(1, repeats + 1):
            ...
            try:
                ...
                print(" ERROR")
                sys.stderr.write(
                    f"[ERROR] speed_sig failed for {alg} run {r} (exit code {e.returncode})\n"
                )
                sys.stderr.write(e.stdout + "\n" + e.stderr + "\n")
                continue
            ops, sizes = parse_speed_sig_output(result.stdout)
            if sizes:
                sizes_map[alg] = sizes
    ...
    for alg in algorithms:
        for op in OPERATIONS:
            ...
            summary_rows.append({
                ...
                "signature_bytes": sizes.get("signature_bytes"),
            })
    fieldnames = [
        "algorithm",
        "operation",
...
def main():
    parser = argparse.ArgumentParser(
        description="Run liboqs ./speed_sig multiple times and aggregate results."
    )
    parser.add_argument(
        "-n",
        "--repeats",
    ...
    parser.add_argument(
        ...
        dest="exec_path",
        default=None,
        help="Path to speed_sig executable (default: ./speed_sig in script directory)",
    )
    parser.add_argument(
        "--alg",
        dest="alg",
    ...
    if args.exec_path is None:
        exec_path = os.path.join(script_dir, "speed_sig")
    else:
        exec_path = args.exec_path
    if not os.path.isfile(exec_path):
        sys.stderr.write(f"[ERROR] speed_sig executable not found at {exec_path}\n")
        sys.exit(1)
    results_dir = os.path.join(script_dir, "results_speed_sig")
    os.makedirs(results_dir, exist_ok=True)
    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    output_csv_path = os.path.join(results_dir, f"results_speed_sig_{ts}.csv")
    if args.alg:
        algorithms = [args.alg]
    else:
